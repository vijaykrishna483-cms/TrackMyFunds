...
import { checkBudgetOverrun } from "../libs/budgetOverrun.js"
    ...
    try {
        ...
        const transactionsResult = await pool.query({
          ...
          res.status(200).json({
            ...
            chartData: data,
            lastTransactions,
            lastAccounts: lastAccount, // ✅ fixed key name
          });
...
export const addTransaction=async (req,res)=>{
    ...
    try {
       ...
       if(!description || !source || !amount){
        ...
        if (Number(amount) <= 0) {
            return res.status(400).json({ status: "failed", message: "Amount must be greater than zero" });
        }
          ...
          await checkBudgetOverrun(userId, description);
...
export const deleteTransaction = async (req, res) => {
  try {
    ...
    // 3. Refund the amount back to the account (if source account exists)
  const accountResult = await pool.query(
  `SELECT * FROM tblaccount WHERE account_name = $1`,
  [transaction.source]  // 'source' holds account_name like "gpay", "cash", etc.
);
    ...
    await pool.query(
      `UPDATE tblaccount SET account_balance = account_balance + $1, updatedat = CURRENT_TIMESTAMP WHERE account_name = $2`,
      [transaction.amount, transaction.source]
    );
    
    // 4. Delete the transaction
    await pool.query(`DELETE FROM tbltransaction WHERE id = $1`, [
      transaction_id,
    ]);
